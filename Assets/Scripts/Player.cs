...
public class Player : MonoBehaviour
{
	...
	void FixedUpdate ()
    {
        ...
        /*for (int i = -1; i < 2; i++)
        {
            Debug.DrawLine(transform.position, transform.position - Vector3.right * (float)i * wallCheckLen);
            RaycastHit2D hit = Physics2D.Raycast(new Vector2(transform.position.x, transform.position.y), Vector2.right * (float)i , wallCheckLen, ~(1 << playerLayer));
            if (hit.collider != null)
            {
    ...
    {
        if (Mathf.Abs(in_normal.x) > Mathf.Abs(in_normal.y))
        {
            rbody.velocity = new Vector2(in_normal.x * moveSpd * 10.0f, rbody.velocity.y);
        }
    }
